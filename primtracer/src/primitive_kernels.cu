...
void init_ray_start_samples(Params* params, OptixAabb* aabbs, int* d_hit_count, int* d_hit_inds) {
    // Note: means/scales/ray_origins/ray_directions are stored as flat float arrays
    // with 3 floats per element (packed float3 layout)
    const uint64_t num_prims = params->means.count / 3;

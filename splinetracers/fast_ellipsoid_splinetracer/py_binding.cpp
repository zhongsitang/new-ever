...
public:
  ...
  fesOptixContext(const torch::Device &device) : device(device.index()) {
    ...
    device_desc.enableValidation = false;
    device = rhi::getRHI()->createDevice(device_desc);
    if (!device) {
      throw std::runtime_error("Failed to create slang-rhi CUDA device.");
    }
    queue = device->getQueue(rhi::QueueType::Graphics);
  }
  ~fesOptixContext() = default;
};
...
public:
  ...
  fesPyGas(const fesOptixContext &context, const torch::Device &device,
      ...
      : gas(context.device, context.queue, device.index(), model.model, enable_anyhit,
            fast_build) {}
};
...
public:
  ...
  fesPyForward(const fesOptixContext &context, const torch::Device &device,
      ...
      : device(device),
        forward(context.device, context.queue, device.index(), model.model, enable_backward),
  ...
  py::dict trace_rays(const fesPyGas &gas, const torch::Tensor &ray_origins,
    ...
    const size_t ray_count = ray_origins.numel() / 3;
    torch::Tensor color;
    color = torch::zeros({(long)ray_count, 4},
                         torch::device(device).dtype(torch::kFloat32));
    torch::Tensor tri_collection =
        torch::zeros({(long)(ray_count * max_iters)},
                     torch::device(device).dtype(torch::kInt32));
    ...
    fesSavedForBackward saved_for_backward(ray_count, num_prims, device);
    forward.trace_rays(gas.gas.get_tlas(), ray_count,

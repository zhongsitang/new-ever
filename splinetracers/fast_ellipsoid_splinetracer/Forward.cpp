...
Forward::Forward(OptixDeviceContext context, int8_t device,
    ...
    : context_(context), device_(device), enable_backward_(enable_backward), model_(&model)
{
    ...
    OptixProgramGroupDesc hit_desc = {
        ...
        .hitgroup = {
            .moduleAH = module_, .entryFunctionNameAH = "__anyhit__ah",
    ...
    for (auto pg : pgs) OPTIX_CHECK(optixUtilAccumulateStackSizes(pg, &stack, pipeline_));
...
void Forward::trace_rays(OptixTraversableHandle handle, size_t num_rays,
                         ...
                         int* tri_collection, int* d_touch_count, int* d_touch_inds)
{
    ...
    if (camera) params_.camera = *camera;
    ...
    CUDA_CHECK(cudaMemcpy(reinterpret_cast<void*>(d_params_), &params_, sizeof(Params), cudaMemcpyHostToDevice));

...
// Forward declarations of CUDA kernels generated by slangc
// These are defined in the generated backward_kernel.cu
extern "C" {
    void backwards_kernel_launch(
        dim3 gridDim,
        dim3 blockDim,
        cudaStream_t stream,
        float* last_state,
...
public:
    ...
    py::dict backward(
        ...
        if (iters.sum().item<int>() > 0) {
            ...
            backwards_kernel_launch(
                dim3(grid_size, 1, 1),
                dim3(block_size, 1, 1),
                stream,
                states.contiguous().data_ptr<float>(),
                diracs.contiguous().data_ptr<float>(),
                iters.contiguous().data_ptr<int>(),
                tri_collection.contiguous().data_ptr<int>(),
                ray_origins.contiguous().data_ptr<float>(),
                ray_directions.contiguous().data_ptr<float>(),
                // DualModel fields
                ...
                dL_dray_origins.data_ptr<float>(),
                dL_dray_dirs.data_ptr<float>(),
                dL_dmeans2D.data_ptr<float>(),
                // Other params
                initial_drgb.contiguous().data_ptr<float>(),
                dL_dinitial_drgb.data_ptr<float>(),
                touch_count.data_ptr<int>(),
                dL_doutputs.contiguous().data_ptr<float>(),
                wcts.contiguous().data_ptr<float>(),
                tmin,
                tmax,
                max_prim_size,
                static_cast<unsigned int>(max_iters),
                static_cast<unsigned int>(num_rays),
                static_cast<unsigned int>(num_prims),
                static_cast<unsigned int>(feature_size)
            );
        }
